...
static constexpr float SUPPLY_TEMP_OFFSET_C =
    1.0f * 5.0f / 9.0f; // ~0.556°C (1°F)
static constexpr float RETURN_TEMP_OFFSET_C =
    1.0f * 5.0f / 9.0f; // ~0.556°C (1°F)
...
static constexpr float SUPPLY_COOL_MAX_C =
    18.0f; // Supply ≤ 18°C supports cooling
static constexpr float SUPPLY_HEAT_MIN_C =
    30.0f; // Supply ≥ 30°C supports heating
...
static constexpr uint32_t NETWORK_SEND_INTERVAL_MS =
    15000; // 15 sec for WiFi (future)
